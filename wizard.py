...
global language
libserver = "https://api.deepl.com/v2/translate"
...
#################################################################
friendlymodelname = "okere/suppple"  # local kobold api apparently needs a hardcoded known HF model name
maxctx = 2048
maxhordectx = 2048
maxhordelen = 256
...
class ServerRequestHandler(http.server.SimpleHTTPRequestHandler):
    ...
    async def generate_text(self, genparams, api_format, stream_flag):
        ...
        syslang="en"
        def run_blocking():
            nonlocal language  #api format 1=basic,2=kai,3=oai,4=oai-chat
            nonlocal syslang
                ...
                for message in messages_array:
                    ...
                    if message['role'] == 'system':
                        ...
                        if detected_language != 'en':
                            ...
                            response = requests.post(libserver, headers=headers, data=json.dumps(payload))
                            print(response.json())
                            syslang = response.json()["translations"][0]["detected_source_language"]
                            print(f"this is system language:{syslang}")
                    ...
                    if message['role'] == 'user' and syslang != "EN" :
                        print(f"Detected language: {detected_language}")
                        
                        if detected_language != 'en':
                            print("translating user")
                            ...
                            print(f"this is user language:{language}")
            ...
            return generate(
                ...
                max_length=genparams.get('max_length', 600),
                temperature=genparams.get('temperature', 0.7),
                top_k=genparams.get('top_k', 600),
        ...
        if api_format==1:
            ...
            if  syslang != "EN":
                            ...
                            response = requests.post(libserver, headers=headers, data=json.dumps(payload))
                            print(response.json())
                            recvtxt = response.json()["translations"][0]["text"]

                            res = {"id": "chatcmpl-1", "object": "chat.completion", "created": 1, "model": "suppple",
                            "usage": {"prompt_tokens": 100,"completion_tokens": 100,"total_tokens": 200},
...
<input type="submit" value="Reset">
    ...
    def do_GET(self):
        ...
        elif self.path.endswith(('/api/v1/check', '/api/latest/model')):
...
def main(launch_args,start_server=True):
    ...
    if not args.remotetunnel:
        print(f"Starting Api {args.port} at {epurl}/v1/")
